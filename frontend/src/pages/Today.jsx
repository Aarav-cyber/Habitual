...
export default function Today() {
  ...
  const completedCount = habits.filter(h => entryFor(h._id)?.completed).length;
    ...
    <div style={{ maxWidth: 680, margin: '0 auto', padding: '32px 24px' }}>
      ...
      {todayHabits.length > 0 && (
        <div className="glass fade-up" style={{ padding: '16px 20px', marginBottom: 20 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
            <span style={{ fontSize: 14, fontWeight: 600 }}>{completedCount} / {todayHabits.length} completed</span>
          ...
          {pct === 100 && (
            <div style={{ marginTop: 10, fontSize: 13, color: 'var(--green)', fontWeight: 600 }}>
              🎉 Perfect day! You nailed all habits.
            </div>
          )}
        </div>
      )}
      ...
      {habits.length === 0 && (
        <div style={{ textAlign: 'center', padding: '60px 20px', color: 'var(--muted)' }}>
          ...
          <p style={{ fontSize: 14, marginBottom: 24 }}>Build consistency one day at a time.</p>
